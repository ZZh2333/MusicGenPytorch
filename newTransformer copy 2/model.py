...
from mask import mask_pad, mask_tril, encoder_mask_pad
from util import MultiHead, PositionEmbedding, FullyConnectedOutput, EncoderPositionEmbedding, encoderMultiHead

# 编码器层
class EncoderLayer(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.mh = encoderMultiHead()
...
class Transformer(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.embed_x = EncoderPositionEmbedding()
        self.embed_y = PositionEmbedding()
        self.encoder = Encoder()
        self.decoder = Decoder()
        self.fc_out = torch.nn.Linear(32, 133)

    def forward(self, x, y):
        # [b, 1, 50, 50]
        # encoder_mask_pad_x = encoder_mask_pad(x)
        mask_pad_x = encoder_mask_pad(x)
